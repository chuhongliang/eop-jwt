...
const unless = require('eop-unless');
const helper = require('./helper');
const resolveCookies = helper.authCookie;
const resolveAuthHeader = helper.resolveAuthHeader;
	...
	const middleware = async function jwt(ctx, next) {
		...
		try {
			if (typeof secret === 'function') {
				secret = await helper.getSecret(secret, token)
			}
			...
			const decodedTokens = secrets.map(async (s) => {
				return await helper.verify(token, s, opts)
			});
